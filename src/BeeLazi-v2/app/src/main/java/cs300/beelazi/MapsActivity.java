...
import cs300.beelazi.MapModules.DirectionFinder;
import cs300.beelazi.MapModules.DirectionFinderListener;
import cs300.beelazi.MapModules.GPSTracker;
import cs300.beelazi.MapModules.MapWrapperLayout;
import cs300.beelazi.MapModules.OnInterInfoWindowTouchListener;
import cs300.beelazi.MapModules.Route;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, DirectionFinderListener, GeoTask.Geo {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        //Marking process
        if (map_type==1) {
            eventPoint = new LatLng(mPlace.getCoord().getLatitude(), mPlace.getCoord().getLongitude());
//            eventPoint = new LatLng(DetailActivity.abcList.get(position).getCoord().getLatitude(),DetailActivity.abcList.get(position).getCoord().getLongitude());
            Log.d("EventPoint", eventPoint.latitude + " " + eventPoint.longitude);
            MarkPlace(-1);
        } else {
            loadPeopleData();
            nearbySearch();
        ...
        mMap.setInfoWindowAdapter(new GoogleMap.InfoWindowAdapter() {
            ...
            public View getInfoContents(Marker marker) {
                ...
                if (pos == -1) {
                    tvMarkerName.setText("Name: " + mPlace.getName());
                    ...
                    tvMarkerName.setText("Name: " + placesList.get(pos).getName());
                    tvMarkerInfo1.setText("Rating: ★★★★★");
                    tvMarkerInfo2.setText("Email: ");
                    Picasso.with(MapsActivity.this)
                            .load(placesList.get(pos).getImgUrl())
                            .resize(100, 100)
                            .centerCrop()
                            .into(ivAvatar);
    ...
    private void getData() {
        ...
        map_type = Integer.parseInt(intent.getStringExtra("map_type"));
        Log.d("map_type", String.valueOf(map_type));
        if (map_type==1) {
            position = Integer.parseInt(intent.getStringExtra("position"));
            mPlace = DetailActivity.abcList.get(position);

            Log.d("address", String.valueOf(mPlace.getAddress()));
...
//            try {
//                addresses = geocoder.getFromLocationName(mPlace.getAddress(), 1);
//                Log.d("jjjjjjjj", String.valueOf(addresses.size()));
//                if(addresses.size() > 0) {
//                    latitude= addresses.get(0).getLatitude();
//                    longitude= addresses.get(0).getLongitude();
//
//                    mPlace.setCoord(new MapCoord(latitude, longitude));
...
//            }
            ...
            Log.d("Data position", "mPlace " + mPlace.getCoord().getLatitude() + " " + mPlace.getCoord().getLongitude());
//            Log.d("Data position ne", String.valueOf(position));
        }
    ...
    private void loadPeopleData() {
        //landmarks = DetailActivity.LandmarkList;
    }
    ...
    private void nearbySearch() {
        ...
        for (int i=0; i<placesList.size(); i++) {
            strOrigins = strOrigins + placesList.get(i).getCoord().getLatitude() + "," + placesList.get(i).getCoord().getLongitude() + "|";
        }
    ...
    public void getResult(JSONObject jsonObject) {
        ...
        for (int i=0; i<Dist.size(); i++){
            ...
            {
                eventPoint = new LatLng(placesList.get(i).getCoord().getLatitude(),placesList.get(i).getCoord().getLongitude());
                MarkPlace(i);
            }
        }
