...
import cs300.beelazi.Azimuth.AugmentedPOI;
import cs300.beelazi.Azimuth.MyCurrentAzimuth;
import cs300.beelazi.Azimuth.MyCurrentLocation;
import cs300.beelazi.Azimuth.OnAzimuthChangedListener;
import cs300.beelazi.Azimuth.OnLocationChangedListener;
...
public class ARDirection extends AppCompatActivity implements SurfaceHolder.Callback, OnLocationChangedListener, OnAzimuthChangedListener {
    ...
    private void LoadData(){
      /*  int position = Integer.parseInt(getIntent().getStringExtra("position"));
        FoodModel foodModel = (FoodModel) StaticData.listSuggest.getListModel().get(position);
        int num = foodModel.getAddress().size();
        Log.d("fuckyou", String.valueOf(num));
        ArrayList<Place> placesList = new ArrayList<>();
        MapCoord coord = new MapCoord(10.754580, 106.668752);

        for (int i=0; i<num; i++){
            placesList.add(new Place(foodModel.getPlaceName().get(i), foodModel.getAddress().get(i), foodModel.getCost().get(i),
                    "01264560570", coord, foodModel.getAddressURL().get(i)));
        }*/

        placesList = DetailActivity.abcList;
        Log.d("list size", String.valueOf(placesList.size()));
        for (int i=0; i<placesList.size(); i++)
        {
            double midAzimuth = calculateTeoreticalAzimuth(placesList.get(i));
            ...
            Log.d(placesList.get(i).getName(), String.valueOf(midAzimuth));
        }
    }
    ...
    private void InitItemLayout() {
        for (int i=0; i<placesList.size(); i++) {
            ...
            Picasso.with(this)
                    .load(placesList.get(i).getImgUrl())
                    .resize(60, 60)
                    .centerCrop()
                    .into(ivAvatar);
            TextView tvName = (TextView) layout.findViewById(R.id.tv_ardir_name);
            tvName.setText(placesList.get(i).getName());

            final int finalI = i;
            layout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    /*Intent intent = new Intent(ARDirection.this, LandmarkDetails.class);
    ...
    public double calculateTeoreticalAzimuth(Place mlandmark) {
        double dX = mlandmark.getCoord().getLatitude() - mMyLatitude;
        double dY = mlandmark.getCoord().getLongitude() - mMyLongitude;
    ...
    public void onAzimuthChanged(float azimuthChangedTo) {
        ...
        Log.d("Real Azi", String.valueOf(mAzimuthReal));
        Log.d("ranges size", String.valueOf(ranges.size()));
        for (int i=0; i<ranges.size(); i++) {
            View layout = infoBox.get(i);
            if (ranges.get(i).isInside(mAzimuthReal)) {
                Log.d("Entered", placesList.get(i).getName());
                ...
                Location.distanceBetween(mMyLatitude, mMyLongitude,
                        placesList.get(i).getCoord().getLatitude(), placesList.get(i).getCoord().getLongitude(), dist);
